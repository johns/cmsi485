...
class NaiveBayesClassifier:
    """
    Naive Bayes Classifier that preprocesses existing data to train an agent
    such that the agent can learn how to accurately predict a feature:
    specifically, we are determining whether or not an individual earns more or less
    than 50,000 USD per year when we know some related features about those persons.
    """
...
class NaiveBayesClassifierTests(unittest.TestCase):
    ...
    def test(self):
        ...
        processed_training_data = nbc.preprocess("income-training.csv")
        processed_test_data = nbc.preprocess("income-test.csv")

        trained_model = nbc.train_model(processed_training_data)
